            ...
            <table class="table table-bordered table-striped">
				...
				<tbody>
					<?php foreach ($monthCalendars as $monthCalendar): ?>
						<tr>
							<td><?= $this->Number->format($monthCalendar->id) ?></td>
							<td><?= $monthCalendar->name ?></td>
							<td><?= $this->Html->link('Editar', ['action' => 'edit', $monthCalendar->id]) ?></td>
							<td><?= $this->Form->postLink('Deletar',
								['action' => 'delete', $monthCalendar->id],
								['confirm' => __('Tem certeza que deseja deletar o(a) monthCalendar {0}?', $monthCalendar->id)]) ?>
							</td>
						</tr>
                	<?php endforeach; ?>
                </tbody>
